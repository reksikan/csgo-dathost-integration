...
class DathostWebhookRouter:
    ...
    def __init__(
        ...
        router.add_api_route(MATCH_END_WEBHOOK_ROUTING_KEY, self._match_ended, methods=['POST'])
        router.add_api_route(ROUND_END_WEBHOOK_ROUTING_KEY, self._round_ended, methods=['POST'])
        self._router = router
    ...
    async def _match_ended(
        ...
        match = await self._db_manager.get_match(updated_match.id)
        if match and str(match.secret_key) == authorization:
